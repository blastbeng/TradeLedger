...
class TradingEngine:
    ...
    async def _process_symbol(self, symbol_entry: Dict[str, str], trading_paused: bool = False):
        ...
        try:
            ...
            # --- Programmatic confidence adjustment (backtest + indicator alignment) ---
            if signal.action in ("BUY", "SELL") and not is_critical:
                bt_adjustment = self._compute_backtest_confidence_adjustment(backtest_stats)

                ind_alignment = self._compute_indicator_alignment_score(
                ...
                # Blend: 40% LLM confidence, 30% backtest, 30% indicator alignment
                adjusted = 0.40 * original_confidence + 0.30 * bt_adjustment + 0.30 * ind_alignment
                adjusted = max(0.0, min(1.0, adjusted))

                if abs(adjusted - original_confidence) > 0.05:
                    logger.info(
                        f"Confidence adjusted for {symbol}: LLM={original_confidence:.2f} → "
                        f"{adjusted:.2f} (backtest={bt_adjustment:.2f}, indicators={ind_alignment:.2f})"
                    )
    ...
    def _compute_backtest_confidence_adjustment(self, backtest_stats: Optional[Dict[str, Any]]) -> float:
        """Compute a confidence adjustment factor (0.0-1.0) from backtest results.

        Returns a multiplier that scales the LLM's confidence based on how well
        the strategy parameters perform in historical backtesting.
        """
        if not backtest_stats or backtest_stats.get("insufficient_data") or backtest_stats.get("total_trades", 0) < 3:
            return 1.0  # not enough data — keep LLM confidence as-is
        ...
        # Profit factor
        profit_factor = backtest_stats.get("profit_factor", 0.0)
        ...
        max_dd = backtest_stats.get("max_drawdown_pct", 1.0)
        if max_dd < 0.05:
            score += 1.0
        elif max_dd < 0.10:
            score += 0.7
        ...
        # Sharpe ratio
        sharpe = backtest_stats.get("sharpe_ratio", 0.0)
        if sharpe >= 1.0:
            score += 1.0
        elif sharpe >= 0.5:
            score += 0.7
        ...
        # Total P&L
        total_pnl = backtest_stats.get("total_pnl_pct", 0.0)
        if total_pnl > 0.05:
            score += 1.0
        elif total_pnl > 0.0:
            score += 0.5
        ...
        return max(0.1, min(1.0, score / components))

    def _compute_indicator_alignment_score(
        self,
        action: str,
        rsi: Optional[float],
        macd: Optional[float],
        ...
        """Compute a technical indicator alignment score (0.0-1.0) for the action.

        For BUY: how many indicators are bullish.
        For SELL: how many indicators are bearish.
        """
        ...
        # Bollinger position
        if bb_upper is not None and bb_lower is not None and bb_middle is not None and current_price > 0:
            total += 1
        ...
        # Parabolic SAR
        if parabolic_sar is not None and current_price > 0:
            total += 1
            if parabolic_sar < current_price:
                bullish += 1
            else:
                bearish += 1

        # Ichimoku cloud
        if ichimoku is not None and current_price > 0:
        ...
        if action == "BUY":
            return max(0.0, min(1.0, bullish / total))
        else:
            return max(0.0, min(1.0, bearish / total))
