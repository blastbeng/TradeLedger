...
class TradingEngine:
    ...
    async def _process_symbol(self, symbol_entry: Dict[str, str], trading_paused: bool = False):
        ...
        try:
            ...
            if await self._signal_processor.check_trade_filters(
            ...
            if symbol in self.positions and signal.strategy_params:
                await self._position_manager.update_position_params(
                    symbol,
            ...
            if validated.action != "HOLD":
                ...
                if validated.action == "BUY":
                    if await self._signal_processor.check_sector_concentration(
    ...
    async def _check_risk_management(self):
        ...
        for symbol, pos in list(self.positions.items()):
            try:
                ...
                # --- Partial take-profit ---
                await self._risk_manager.check_partial_take_profit(
                ...
                if await self._risk_manager.check_native_exit_triggers(
    ...
    async def _execute_signal(self, symbol: str, signal, timeframe: str = None, exit_reason: str = None, atr: Optional[float] = None):
        ...
        if signal.action == "BUY":
            ...
            _sizing_result = await self._order_executor.compute_position_size(
                symbol=symbol,
                display_symbol=display_symbol,
            ...
            try:
                ...
                async with self._cycle_spent_lock:
                    ...
                    self._cycle_spent += amount
                await self._order_executor.update_or_create_buy_position(
                    symbol=symbol,
                    order=order,
                    signal=signal,
                    params=params,
