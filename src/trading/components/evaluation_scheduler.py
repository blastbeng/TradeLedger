...
class EvaluationScheduler:
    ...
    async def get_symbols_to_process(self, now: float) -> List[Dict[str, str]]:
        ...
        # on missing clock). User preference: trading AND re-evaluation are fully
        # paused when the market is closed — NO forced path may issue LLM calls.
        if not await is_llm_active_now(engine.event_bus):
            if engine._force_reeval or engine._reeval_pending_force:
                logger.info("Market closed; trading and re-evaluation are paused — forced re-evaluation deferred until pre-market/open.")
                engine._reeval_pending_force = False
        ...
        # (Market-closed forced-eval restriction removed per user preference:
        # when the market is closed, get_symbols_to_process returns [] early —
        # trading and re-evaluation are fully paused, no LLM calls at all.)
