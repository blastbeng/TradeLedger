...
- **Utilize all available symbol slots aggressively.** You have been allocated a maximum number of symbols (MAX_SYMBOLS). You should actively try to select up to that maximum when there are profitable opportunities. Do not default to selecting only 2–3 symbols when you have many slots available — each unused slot is a missed opportunity for profit. Stocks, ETFs, and BTPs all have **equal priority** — do not favor one asset class over another. Select the best opportunities across all asset classes to fill your available slots. A diversified portfolio of many small positions is often better than concentrating on a few large positions, because it spreads risk and increases the chance of capturing profitable moves. When you have 50 slots available, selecting only 3 wastes 47 potential opportunities. Aim to fill as many slots as possible with small position sizes (e.g., position_size_fraction of 0.02–0.05 each) rather than concentrating capital in a handful of trades.
...
- **Two-Step Decision Process with Multiple Backtest Variants:** You will now operate in two steps. 
  1. In the first step, you will analyze the market data, indicators, and statistical summaries, and propose **multiple** sets of strategy parameters for backtesting. Each set is called a "backtest variant" and should explore a different hypothesis (e.g., tight stop vs wide stop, short hold vs long hold, trailing stop on/off, different take-profit targets, etc.). You decide how many variants to return (minimum 1, recommended 3–5, maximum 10). The engine will run a local Python backtest for EACH variant sequentially. If you provide more than 10 variants, only the first 10 will be tested. Running just one backtest may not be enough to intercept profitable configurations, so provide several diverse variants to maximize the chance of finding a winning strategy.
...
"""
    ...
    if historical_ohlcv:
        ...
        prompt += (
            ...
            "**Backtest Entry Logic (Customizable):** By default, the backtester enters a long position at the close of every candle "
            "if the trend is up (close > EMA50) AND the trend is strong (ADX > 20). "
            "**You CAN customize this entry logic** by including a `backtest_entry_config` object in each backtest variant's parameters. "
            "This allows you to backtest the strategy with entry conditions that match your actual intended entry logic.\n"
            "The `backtest_entry_config` object supports these fields (all optional — defaults shown):\n"
            "- `ema_period` (int, default 50): EMA period for trend filter. Set to 0 to disable EMA filter.\n"
            "- `ema_direction` (\"above\" or \"below\", default \"above\"): enter when close is above/below the EMA.\n"
            "- `min_adx` (float, default 20): minimum ADX to enter. Set to 0 to disable ADX filter.\n"
            "- `max_rsi` (float, default 70): maximum RSI to enter (avoids overbought). Set to 100 to disable.\n"
            "- `min_rsi` (float, default 0): minimum RSI to enter (avoids oversold entries). Set to 0 to disable.\n"
            "- `macd_filter` (\"positive\", \"negative\", or \"none\", default \"positive\"): require MACD histogram above/below 0.\n"
            ...
            "If you omit `backtest_entry_config`, the default (close > EMA50 AND ADX > 20) is used.\n"
