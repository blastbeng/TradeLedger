...
- **Do NOT set max_hold_time_seconds too short.** A too-short max hold time forces an exit before the trade has time to develop. Err on the side of longer hold times. For 1h candles, consider at least 1-3 days; for 1d candles, 1-2 months; for 1w candles, 3-6 months.
- **Required parameter for every BUY/SELL:**
...
Select between 0 and {max_symbols} assets (stocks/ETFs or BTP bonds) to trade. The available symbols may include Italian BTP bonds identified by their ISIN (e.g., IT0001234567). The `name` field in the market data contains the bond's description, including maturity and coupon (e.g., 'Btp-1nv26 7,25%' means November 2026 maturity, 7.25% coupon). You can select them alongside stocks. If market conditions are extremely unfavorable (e.g., high losses, poor momentum, negative sentiment), you may select 0 assets to pause trading until the next evaluation. You decide the exact number based on how many high‑quality opportunities you see. If market conditions are poor, you may choose fewer assets (even 0 or 1) to concentrate capital on the best setup. If many strong setups exist, you may select up to {max_symbols}. You MUST only select assets where the per-asset budget ({per_symbol_budget:.2f} {base_currency}) is greater than or equal to the asset's min_trade_cost. Skip any asset that does not meet this requirement. Prefer stocks with high volume and positive momentum. You may keep some current assets if they are still promising and meet the budget requirement, or replace them. **Prefer to keep assets that have been tracked for a while** – they have more historical data and the bot has already invested in learning their behaviour. Only drop an asset if it shows clear deterioration (e.g., negative momentum on all timeframes, poor win rate, or strongly negative sentiment). For assets already being tracked, re-evaluate their assigned timeframe. If the market regime has changed (e.g., a stock that was trending on 1h is now choppy and better suited to 15m), update the timeframe. If you change the timeframe for an asset with an open position, the bot will switch to managing the position using the new timeframe.
