...
def _get_max_input_tokens(provider: str, model_type: str, is_fallback: bool) -> int:
    ...
    if provider == "openai":
        ...
        if is_fallback:
            if model_type == "mind":
                return settings.G4F_MIND_FALLBACK_MAX_INPUT_TOKENS
            elif model_type == "weak":
                return settings.G4F_WEAK_FALLBACK_MAX_INPUT_TOKENS
            else:
                return settings.G4F_ACTUATOR_FALLBACK_MAX_INPUT_TOKENS
        else:
            if model_type == "mind":
                return settings.G4F_MIND_MAX_INPUT_TOKENS
            elif model_type == "weak":
                return settings.G4F_WEAK_MAX_INPUT_TOKENS
            else:
                return settings.G4F_ACTUATOR_MAX_INPUT_TOKENS
    else:  # ollama
        if is_fallback:
            if model_type == "mind":
...
def _get_fallback_provider_config(model_type: str, provider: str = None):
    ...
    if fallback_provider == "openai":
        ...
        from src.llm.g4f_client import _get_g4f_models
        fb_models = _get_g4f_models(model_type)
        fb_base_url = None
        fb_api_key = None
    else:  # ollama
        if model_type == "mind":
